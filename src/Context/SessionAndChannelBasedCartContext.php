...
namespace Gvidasr\ChannelsGroupPlugin\Context;
...
final class SessionAndChannelBasedCartContext implements CartContextInterface
{
    ...
    public function getCart(): OrderInterface
    {
        ...
        if (!$this->cartStorage->hasForChannel($channel)) {
            if(!empty($channel->getChannelGroup()) && $channel->getChannelGroup()->isSharedBasket()) {
                foreach($channel->getChannelGroup()->getChannels() as $siblingChannel) {
                    if($this->cartStorage->hasForChannel($siblingChannel)) {
                        $cart = $this->cartStorage->getForChannel($siblingChannel);
                        if (null !== $cart) {
